// admin.js (Updated handleFormSubmit to match new form fields)
...
async function isAdminAuthenticated(session) { try { if (!session) { throw new Error('No active session provided.'); } const user = session.user; const { data: profile, error: profileError } = await supabase.from('profiles').select('role, status').eq('id', user.id).single(); if (profileError) { throw profileError; } if (!profile || profile.role !== 'admin' || profile.status !== 'approved') { const reason = !profile ? 'Profile not found' : `Role/Status invalid (role=${profile.role}, status=${profile.status})`; throw new Error(`Access Denied: ${reason}.`); } return true; } catch (error) { console.warn('[AUTH] Admin check failed:', error.message); await supabase.auth.signOut(); return false; } }
async function boot(session) { console.log('[BOOT] Initializing...'); if (await isAdminAuthenticated(session)) { showView('app'); await loadInitialData(); } else { showView('gate'); } }


// --- UI Rendering & Data Fetching ---
...
async function handleFormSubmit(e) {
    ...
    try {
        ...
        // UPDATED: Removed unused fields from this list
        const formFields = [
            'bank_id', 'promotion_name', 'start_date', 'end_date', 'max_ltv', 
            'dsr_limit', 'income_per_million', 'min_living_expense', 'max_income', 
            'max_loan_amount', 'max_loan_tenure', 'max_age_salaried', 
            'max_age_business', 'notes'
        ];
        ...
        // UPDATED: Removed other_income_continuity_required
        promoData.waive_mortgage_fee = document.getElementById('waive_mortgage_fee').checked;
        promoData.has_mrta_option = document.getElementById('has_mrta_option').checked;

        const rateInputs = ratesContainer.querySelectorAll('.rate-input');
...
function setupEventListeners() {
    ...
    supabase.auth.onAuthStateChange((event, session) => { console.log(`[AUTH] Event: ${event}, Session available:`, !!session); scheduleSessionRefresh(session); if (event === 'SIGNED_IN') { if (!appBooted) { appBooted = true; boot(session); } } else if (event === 'SIGNED_OUT') { appBooted = false; showView('gate'); showToast('ออกจากระบบแล้ว'); } });
}
