'use strict';
// admin-rates.js (ASCII-only) - base_rates history + apply-latest + mobile UX
...
function renderHistory(){
  const tb=$('rates-history-rows'); if(!tb) return; tb.innerHTML='';
  for(let i=0;i<state.history.length;i++){
...
function renderLatestSummary(bankId){
  ...
  const el=$('latest-summary'); if(!el) return;
  const parts=['MRR','MLR','MOR'].map(t=>{
    const r=L[t]; return t+': '+(r?fmtRate(r.rate)+'% ('+fmtDate(r.effective_from)+')':'-');
  });
...
async function applyLatestOne(){
  ...
  await fetchBanks(); renderLatestSummary(bankId);
  alert('อัปเดตแล้ว');
}
...
function attach(){
  $('rates-add-btn') && $('rates-add-btn').addEventListener('click',addHistory);
  $('apply-latest-one') && $('apply-latest-one').addEventListener('click',applyLatestOne);
  $('apply-latest-all') && $('apply-latest-all').addEventListener('click',applyLatestAll);
  $('rates-bank-select') && $('rates-bank-select').addEventListener('change',e=>fetchHistory(Number(e.target.value)));
  window.FormUX && window.FormUX.enableEnterNavigation();
  window.FormUX && window.FormUX.enableNumericKeypad('#rates-value');
}
document.addEventListener('DOMContentLoaded', async ()=>{
  attach();
  $('rates-date') && ($('rates-date').value = new Date().toISOString().slice(0,10));
  await fetchBanks();
  const sel=$('rates-bank-select');
