...
async function fetchRules(){
  const sb = await getSupabase();
  const { data, error } = await sb.from('bank_rules').select('*').order('product_type', {ascending:true}).limit(500);
...
function renderRules(rows, banks){
  ...
  rows.forEach(r=>{
    ...
    tr.innerHTML = `
      ...
      <td>
        <select class="br-product" data-id="${r.id}">
          ${PRODUCTS.map(p=>`<option value="${p}">${p}</option>`).join('')}
        </select>
      </td>
      <td>
        <select class="br-prop" data-id="${r.id}">
          ${PROPS.map(p=>`<option value="${p}">${p||'-'}</option>`).join('')}
        </select>
      </td>
      <td>
        <select class="br-home" data-id="${r.id}">
          ${HOMES.map(h=>`<option value="${h}">${h||'-'}</option>`).join('')}
        </select>
      </td>
      ...
      <td>
        <button class="btn br-save" data-id="${r.id}">บันทึก</button>
        <button class="btn outline br-del" data-id="${r.id}">ลบ</button>
      </td>
    `;
    body.appendChild(tr);
...
async function addNewRule(){
  const sb = await getSupabase();
  const { data, error } = await sb.from('bank_rules').insert([{ product_type:'MORTGAGE' }]).select('id');
  if (error) return alert(error.message||'เพิ่มไม่สำเร็จ');
  await refresh();
}

export async function refresh(){
  const [banks, rules] = await Promise.all([listBanks(), fetchRules()]);
  renderRules(rules, banks);
}

export async function initAdminRules(){
  await ensureAdmin();
  qs('#btn-rules-add')?.addEventListener('click', addNewRule);
  qs('#btn-rules-refresh')?.addEventListener('click', refresh);
  await refresh();
}
